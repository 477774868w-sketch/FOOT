...
class RubricAssessment:
    ...
    def render(self) -> str:
        line = f"{self.status.symbol} R{self.rubric.number:02d} {self.rubric.title}"
        if self.summary:
            line += f" — {self.summary}"
        if self.model_variables:
            line += f"  [→ modèle : {', '.join(self.model_variables)}]"
        if self.blocker:
            line += f"  (bloqué : {self.blocker})"
        return line
...
def _r(
    ...
    *,
    ...
    return Rubric(number, title, phase, frozenset(requires), detail, feeds_model)
...
RUBRICS: tuple[Rubric, ...] = (
    ...
    _r(4, "Forme récente (5 à 10 matchs) dans un historique long", RubricPhase.SPORT,
       [Capability.RESULTS], feeds_model=True),
    ...
    _r(15, "Météo, pelouse et arbitre", RubricPhase.SPORT,
        [Capability.WEATHER, Capability.REFEREE]),
